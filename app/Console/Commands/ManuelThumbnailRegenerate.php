...
class ManuelThumbnailRegenerate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'manuel:thumbnail-regenerate {folder? : İşlenecek klasör adı}';
     ...
     * @var string
     */
    protected $description = 'Manuel videolar için thumbnail\'leri yeniden oluşturur';
    ...
    {
        $this->info('Thumbnail yeniden oluşturma işlemi başlatılıyor...');
        ...
        foreach ($groupedFiles as $folderName => $filesInFolder) {
            ...
            foreach ($filesInFolder as $index => $file) {
                ...
                try {
                    // Thumbnail'i yeniden oluştur
                    app(VideoService::class)->generateThumbnail($video, 'manuel-videos');
                    $this->info('  ✅ Thumbnail başarıyla oluşturuldu');
                    $processedCount++;
                } catch (\Exception $e) {
